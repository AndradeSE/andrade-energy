...
import { Alert, RefreshControl, ScrollView as HorizontalScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View } from "react-native";
...
export default function Financeiro() {
  ...
  return <Screen><AppHeader collapsePlantContextOnMount title="Financeiro" subtitle="Receita da carteira" contextTitle={moeda(dados.receitaRecebida)} contextSubtitle={`${dados.percentualRecebido.toFixed(1)}% da receita recebida`} icon="wallet-outline" />
    {loading ? <Loading /> : <ScrollView bounces alwaysBounceVertical overScrollMode="always" refreshControl={<RefreshControl refreshing={atualizando} onRefresh={atualizarPagina} tintColor={Colors.primary} colors={[Colors.primary]} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Section title="Faturamento" framed={false}><HorizontalScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.quickCarousel}>
...
const styles = StyleSheet.create({
  ...
  quickCarousel: { gap: Spacing.sm, paddingRight: Spacing.lg }, quickAction: { width: 150, minHeight: 112, alignItems: "center", justifyContent: "center", padding: Spacing.sm, borderWidth: 1, borderColor: "#C9DED1", borderRadius: Radius.lg, backgroundColor: Colors.surface }, quickActionActive: { borderColor: Colors.primary, backgroundColor: Colors.primaryLight }, quickIcon: { width: 44, height: 44, alignItems: "center", justifyContent: "center", borderRadius: Radius.md, backgroundColor: Colors.primaryLight }, quickLabel: { marginTop: Spacing.sm, color: Colors.text, fontSize: Typography.small, fontWeight: "800", textAlign: "center" }, sectionLead: { marginBottom: Spacing.sm, color: Colors.subtitle, fontSize: Typography.small, lineHeight: 19 },
