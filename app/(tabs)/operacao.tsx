  ...
  return <Screen><AppHeader title="Operação" subtitle="Fechamento mensal" contextTitle={competencia ? `Competência ${competenciaNome(competencia)}` : "Operação mensal"} contextSubtitle="Geração, rateio, faturamento e recebimentos" icon="analytics-outline" />
    {loading ? <Loading /> : <FlatList bounces alwaysBounceVertical overScrollMode="always" refreshControl={<RefreshControl refreshing={atualizando} onRefresh={atualizarPagina} tintColor={Colors.primary} colors={[Colors.primary]} />} contentContainerStyle={styles.content} data={historico} keyExtractor={(item) => item.id}
      ListHeaderComponent={<View>
        ...
        <Card style={styles.conferenceCard}><View style={styles.sectionHeading}><Ionicons name="checkmark-done-outline" size={20} color={Colors.primary} /><View><Text style={styles.title}>Conferência</Text><Text style={styles.subtitle}>Acompanhe o processamento desta competência.</Text></View></View><View style={styles.grid}>
        ...
        <View style={styles.adjustmentButton}><Button title="Registrar ajuste excepcional" icon={<Ionicons name="create-outline" size={20} color={Colors.surface} />} onPress={() => router.push("/operacao/novo")} /></View>
        <Card><Text style={styles.title}>Usinas nesta competência</Text><Text style={styles.subtitle}>Selecione uma usina para conferir o fechamento.</Text></Card>
      </View>}
