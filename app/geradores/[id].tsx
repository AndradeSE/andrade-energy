...
export default function DetalhesGerador() {
  ...
  function confirmarRemocao() {
    ...
    Alert.alert("Remover gerador", `Remover o acesso de ${gerador?.nome ?? "este gerador"}? A assinatura será cancelada e o histórico financeiro será preservado.`, [
      ...
      { text: "Remover", style: "destructive", onPress: async () => {
        try {
          setRemovendo(true);
          await removerGerador(String(id));
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content}><TouchableOpacity onPress={()=>router.back()} style={styles.back}><Ionicons name="arrow-back" size={22} color={Colors.primaryDark}/></TouchableOpacity><View style={styles.hero}><View style={styles.avatar}><Text style={styles.avatarText}>{String(gerador.nome??"G").charAt(0).toUpperCase()}</Text></View><Text style={styles.eyebrow}>INFORMAÇÕES DO GERADOR</Text><Text style={styles.title}>{gerador.nome}</Text><Text style={styles.subtitle}>{gerador.email ?? "E-mail não informado"}</Text><View style={styles.status}><Text>{gerador.perfil === "ADMIN" ? "ADMINISTRADOR · FIXO" : assinatura?.status ?? (gerador.ativo ? "SEM ASSINATURA" : "INATIVO")}</Text></View></View><View style={styles.grid}>{metrics.map(([icon,label,value])=><View style={styles.metric} key={label}><Ionicons name={icon as any} size={22} color={Colors.primary}/><Text style={styles.metricLabel}>{label}</Text><Text style={styles.metricValue}>{value}</Text></View>)}</View><View style={styles.info}><Text style={styles.infoTitle}>Dados da conta</Text><Row label="Nome completo" value={gerador.nome ?? "Não informado"}/><Row label="E-mail" value={gerador.email ?? "Não informado"}/><Row label="Telefone" value={gerador.telefone ?? "Não informado"}/><Row label="CPF" value={gerador.cpf ?? "Não informado"}/><Row label="Perfil" value={gerador.perfil ?? "GESTOR"}/><Row label="Cadastro" value={dataBr(gerador.created_at)}/><Row label="Início do acesso" value={dataBr(assinatura?.inicio_em)}/><Row label="Situação da conta" value={gerador.ativo ? "Ativa" : "Inativa"}/></View>{gerador.perfil === "ADMIN" ? <View style={styles.protected}><Ionicons name="shield-checkmark-outline" size={20} color={Colors.primary}/><Text style={styles.protectedText}>Conta administrativa fixa e protegida contra remoção.</Text></View> : <TouchableOpacity disabled={removendo} onPress={confirmarRemocao} style={[styles.removeButton, removendo && styles.disabled]}><Ionicons name="trash-outline" size={20} color={Colors.danger}/><Text style={styles.removeText}>{removendo ? "Removendo..." : "Remover gerador"}</Text></TouchableOpacity>}</ScrollView></Screen>;
}
