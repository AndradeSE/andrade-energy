...
export default function ClienteDetalhe() {
  ...
  const concessionariaCliente = String(
    unidades.find((unidade) => String(unidade?.distribuidora ?? "").trim())?.distribuidora
      ?? cliente.distribuidora
      ?? "",
  ).trim();
  const usinasAlocadas = Array.from(new Set(
    unidades
      .map((unidade) => String(unidade?.usinas?.nome ?? unidade?.usina_nome ?? "").trim())
      .filter(Boolean),
  )).join(", ");
