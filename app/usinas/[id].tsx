...
export default function DashboardUsina() {
  ...
  return <Screen>
    {IS_GERADOR_APP ? <AppHeader title="Usinas" subtitle="Gestão de geração" contextTitle={data.usina?.nome ?? "Usina"} contextSubtitle={`UC ${data.usina?.numero_instalacao ?? "não informada"} · Competência ${data.competencia}`} icon="sunny-outline" /> : null}
