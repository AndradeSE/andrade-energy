...
import { Badge, Button, Card, Divider, EmptyState, Loading, Metric, Screen, Section } from "../../components/ui";
...
export default function DashboardUsina() {
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content}><View style={styles.heading}><View><Text style={styles.eyebrow}>DETALHES DA USINA</Text><Text style={styles.title}>{data.usina?.nome ?? "Usina"}</Text><Text style={styles.subtitle}>Competência {data.competencia}</Text></View><Badge label={data.status ?? "ATIVA"} variant="success" /></View>
    ...
    <Button title="Alocar clientes" icon={<Ionicons name="people-outline" size={20} color={Colors.surface} />} onPress={() => router.push({ pathname: "/usinas/alocar", params: { id, nome: data.usina?.nome ?? "Usina" } })} />
    <Button title="Editar dados da usina" icon={<Ionicons name="create-outline" size={20} color={Colors.surface} />} style={styles.editButton} onPress={() => router.push({ pathname: "/usinas/editar", params: { id } })} />
  </ScrollView></Screen>;
}
