...
export default function ClienteHome() {
  ...
  const compensacao = consumo > 0 ? Math.min(100, Math.round((creditos / consumo) * 100)) : 0;
      ...
      >
        <View style={styles.overviewHero}>
          <View style={styles.overviewCopy}>
            ...
            <Text style={styles.overviewCaption}>Sua energia gerando valor desde o início do contrato</Text>
            <View style={styles.monthSaving}><Ionicons name="trending-up-outline" size={17} color="#A7F3D0" /><Text style={styles.monthSavingText}>{moeda(economiaMes)} nesta competência</Text></View>
          </View>
          <View style={styles.compensationOrbit}><Text style={styles.compensationValue}>{compensacao}%</Text><Text style={styles.compensationLabel}>compensado</Text></View>
        </View>
        ...
        <View style={styles.overviewHeader}><Text style={styles.overviewTitle}>Visão geral</Text><Text style={styles.overviewHint}>Competência atual</Text></View>
        <View style={styles.metricsGrid}>
          <View style={styles.metricCard}><View style={styles.metricIcon}><Ionicons name="flash-outline" size={19} color={Colors.primary} /></View><Text style={styles.metricLabel}>Consumo</Text><Text style={styles.metricValue}>{energia(consumo)}</Text></View>
          <View style={styles.metricCard}><View style={styles.metricIcon}><Ionicons name="sunny-outline" size={19} color={Colors.primary} /></View><Text style={styles.metricLabel}>Créditos recebidos</Text><Text style={styles.metricValue}>{energia(creditos)}</Text></View>
          <View style={styles.metricCard}><View style={styles.metricIcon}><Ionicons name="home-outline" size={19} color={Colors.primary} /></View><Text style={styles.metricLabel}>Minha unidade</Text><Text numberOfLines={1} style={styles.metricValue}>{String(data.uc ?? "—")}</Text><Text numberOfLines={1} style={styles.metricNote}>{String(data.distribuidora ?? "Concessionária")}</Text></View>
          <TouchableOpacity activeOpacity={0.84} onPress={() => router.push("/faturas")} style={styles.metricCard}><View style={styles.metricIcon}><Ionicons name="receipt-outline" size={19} color={Colors.primary} /></View><Text style={styles.metricLabel}>Faturas pendentes</Text><Text style={styles.metricValue}>{quantidadeEmAberto}</Text><Text style={styles.metricNote}>{moeda(valorEmAberto)}</Text></TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  overviewHero: { minHeight: 188, flexDirection: "row", alignItems: "center", marginBottom: Spacing.lg, padding: Spacing.lg, borderRadius: Radius.xl, backgroundColor: "#063E31", ...Shadows.card },
  overviewCopy: { flex: 1, paddingRight: Spacing.sm }, overviewEyebrow: { color: "#86EFAC", fontSize: 10, fontWeight: "900", letterSpacing: 1 }, overviewValue: { marginTop: 6, color: "#FFFFFF", fontSize: 32, fontWeight: "900", letterSpacing: -.8 }, overviewCaption: { marginTop: 4, color: "#CDEBDE", fontSize: Typography.small, lineHeight: 18 }, monthSaving: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: Spacing.md }, monthSavingText: { color: "#A7F3D0", fontSize: 11, fontWeight: "800" },
  compensationOrbit: { width: 92, height: 92, alignItems: "center", justifyContent: "center", borderWidth: 8, borderColor: "#22C979", borderRadius: 46, backgroundColor: "rgba(255,255,255,.07)" }, compensationValue: { color: "#FFFFFF", fontSize: 23, fontWeight: "900" }, compensationLabel: { marginTop: 1, color: "#BDEBD5", fontSize: 9, fontWeight: "700" },
  overviewHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: Spacing.xs, marginBottom: Spacing.sm }, overviewTitle: { color: Colors.text, fontSize: 17, fontWeight: "800" }, overviewHint: { color: Colors.subtitle, fontSize: 11, fontWeight: "600" }, metricsGrid: { flexDirection: "row", flexWrap: "wrap", gap: Spacing.sm, marginBottom: Spacing.lg }, metricCard: { width: "48%", minHeight: 128, padding: Spacing.md, borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.lg, backgroundColor: Colors.background }, metricIcon: { width: 34, height: 34, alignItems: "center", justifyContent: "center", borderRadius: Radius.md, backgroundColor: Colors.primaryLight }, metricLabel: { marginTop: Spacing.sm, color: Colors.subtitle, fontSize: 11, fontWeight: "700" }, metricValue: { marginTop: 3, color: Colors.text, fontSize: Typography.body, fontWeight: "900" }, metricNote: { marginTop: 3, color: Colors.primary, fontSize: 10, fontWeight: "700" },
