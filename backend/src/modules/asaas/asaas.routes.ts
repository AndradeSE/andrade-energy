...
asaasRouter.post("/cobrancas/:faturaId",exigirAutenticacao,exigirGestor,exigirRegistroDaEmpresa("faturas","faturaId"),async(req,res)=>{try{return res.json(await criarCobrancaAsaas(req.params.faturaId));}catch(error:any){return res.status(400).json({message:error.message});}});
